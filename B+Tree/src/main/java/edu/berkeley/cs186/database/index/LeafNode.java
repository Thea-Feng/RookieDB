...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int pos = InnerNode.numLessThan(key, keys);
        keys.add(pos, key);
        rids.add(pos, rid);
        if (2 * d >= keys.size()) {
            sync();
        ...
        List<RecordId> leftRecords = rids.subList(0, d);
        List<RecordId> rightRecords = rids.subList(d, 2 * d + 1);
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(proj2): implement
        int d = metadata.getOrder();
        int filMax = Double.valueOf(Math.ceil(d*2*fillFactor)).intValue(); //double?
        int idx = keys.size() > 0 ? 1 : 0;
        while(data.hasNext()){
            if(idx < filMax) {
            ...
            // next iter overflow
            if(idx == filMax && data.hasNext()){
                List<DataBox> newKeys = new ArrayList<>();
                List<RecordId> newRids = new ArrayList<>();
                Pair<DataBox, RecordId> newpair = data.next();
                if(keys.contains(newpair.getFirst())){
                    throw new BPlusTreeException("Duplicate Key.");
                }
                newKeys.add(newpair.getFirst());
                newRids.add(newpair.getSecond());
                Optional<Long> newRightSibling = Optional.empty();
                LeafNode newNode = new LeafNode(metadata, bufferManager, newKeys, newRids,newRightSibling, treeContext);

                Long pageNum = newNode.getPage().getPageNum();
                rightSibling = Optional.of(pageNum);
                sync();
                Pair pushPair = new Pair<>(newKeys.get(0), pageNum);
                return Optional.of(pushPair);
            }
        }
        sync();
        return Optional.empty();
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        int number_of_key_value = buf.getInt();
        for (int i = 0; i < number_of_key_value; ++i) {
