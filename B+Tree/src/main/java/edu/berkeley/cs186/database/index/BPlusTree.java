...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanAll() {
        ...
        Iterator<RecordId> iterator = leafNode.scanAll();
        BPlusTreeIterator bPlusTreeIterator = new BPlusTreeIterator(leafNode, iterator);
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        if (!(leafNode.getKey(key).isPresent())) {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> newPushUpNode=root.put(key,rid);
        if(newPushUpNode.isPresent()){
            DataBox getKeys = newPushUpNode.get().getFirst();
            long getChildren = newPushUpNode.get().getSecond();
            List<DataBox> newKeys = new ArrayList<>();
            newKeys.add(getKeys);
            List<Long> newChildren = new ArrayList<>();
            newChildren.add(this.metadata.getRootPageNum());
            newChildren.add(getChildren);
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        // Use the provided updateRoot() helper method to change
        // the tree's root if the old root splits.
        if(this.metadata.getHeight() != 0 ){
            throw new BPlusTreeException("Tree is not empty.");
        }
        try{
            LeafNode haveLeaf = root.getLeftmostLeaf();
        }catch (Exception e){
            throw new BPlusTreeException("Tree is not empty.");
        }
        while(data.hasNext()){
            Optional<Pair<DataBox, Long>> newPushUpNode=root.bulkLoad(data, fillFactor);
            if(newPushUpNode.isPresent()){
                DataBox getKeys = newPushUpNode.get().getFirst();
                long getChildren = newPushUpNode.get().getSecond();// page number
                List<DataBox> newKeys = new ArrayList<>();
                newKeys.add(getKeys);
                List<Long> newChildren = new ArrayList<>();
                newChildren.add(this.metadata.getRootPageNum());
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        RecordId[] items;
        public BPlusTreeIterator(LeafNode leafNode, Iterator<RecordId> iterator) {
