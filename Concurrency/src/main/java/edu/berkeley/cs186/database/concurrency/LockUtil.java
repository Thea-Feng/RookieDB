...
//        if(!LockType.substitutable(requestType, effectiveLockType)) return;
        if(requestType.equals(LockType.S)){ // S
            LockContext now = parentContext;
            ArrayList<LockContext> parentList = new ArrayList<>();
            while(now != null){
                parentList.add(now);
                now = now.parentContext();
            }
            Collections.reverse(parentList);
            for(LockContext parent: parentList){
            ...
            else lockContext.promote(transaction, LockType.SIX);
