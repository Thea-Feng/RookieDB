...
//            Iterator<LockRequest> requests = waitingQueue.iterator();
            while(!waitingQueue.isEmpty()){
                LockRequest lockRequest = waitingQueue.getFirst();
                Lock l = lockRequest.lock;
                boolean fg = checkCompatible(l.lockType, l.transactionNum);
                if (fg) {
                    if (LockType.substitutable(l.lockType, getTransactionLockType(l.transactionNum)))
                        grantOrUpdateLock(l);
                    lockRequest.transaction.unblock();
                    waitingQueue.pollFirst();
//                    iterator need to delete queue or not?
                } else break;
            }
            // TODO(proj4_part1): implement
            return;
    ...
    public void acquireAndRelease(TransactionContext transaction, ResourceName name,
        ...
        // releasename = a list of resource name to be release by that transaction
        boolean shouldBlock = false;
        synchronized (this) {
            if(!getLockType(transaction, name).equals(LockType.NL) && !releaseNames.contains(name))
                throw new DuplicateLockRequestException("There is a lock already held");
            List<Lock> xactLocks = getLocks(transaction);
            HashMap<ResourceName, Lock> xactLocksRes = new HashMap<>();
            for(Lock l : xactLocks) xactLocksRes.put(l.name, l);
            for(ResourceName n : releaseNames){
                if(!xactLocksRes.containsKey(n))
                    throw new NoLockHeldException("There is no such lock to release");
            }

            ResourceEntry resourceEntry = getResourceEntry(name);
            boolean fg = resourceEntry.checkCompatible(lockType, transaction.getTransNum());
            ...
            if(fg) {
                ...
                for(ResourceName n : releaseNames){
                    if(!n.equals(name)){
                        Lock lock1 = xactLocksRes.get(n);
                        getResourceEntry(n).releaseLock(lock1);
                    }
    ...
    public void acquire(TransactionContext transaction, ResourceName name,
        ...
        synchronized (this) {
            ...
            if(fg && resourceEntry.waitingQueue.isEmpty()) {
                resourceEntry.grantOrUpdateLock(lock);
            } else {
                shouldBlock = true;
                transaction.prepareBlock();
    ...
    public void release(TransactionContext transaction, ResourceName name)
        ...
        synchronized (this) {
            ...
            HashMap<ResourceName, Lock> xactLocksRes = new HashMap<>();
            for(Lock l : xactLocks) xactLocksRes.put(l.name, l);
            if(!xactLocksRes.containsKey(name))
                throw new NoLockHeldException("There is no such lock to release");
            Lock lock = xactLocksRes.get(name);
    ...
    public void promote(TransactionContext transaction, ResourceName name,
        ...
        boolean shouldBlock = false;
        synchronized (this) {
            LockType originalType = getLockType(transaction, name);
            if(originalType.equals(newLockType))
                throw new DuplicateLockRequestException("The same lock is held on resource");
            if(originalType.equals(LockType.NL))
                throw new NoLockHeldException("There is no lock held");
            if(!LockType.substitutable(newLockType, originalType))
                throw new InvalidLockException("The new lock type is not a promotion");


            ResourceEntry resourceEntry = getResourceEntry(name);
            boolean fg = resourceEntry.checkCompatible(newLockType, transaction.getTransNum());
