...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        for(int idx : getEligibleIndexColumns(table)) {
            SelectPredicate predicate = selectPredicates.get(idx);
            QueryOperator op = new IndexScanOperator(this.transaction, table, predicate.column,
                    predicate.operator, predicate.value);
            int newIO = op.estimateIOCost();
            if(newIO < minIO) {
                minIO = newIO;
                minOp = op;
                columNum = idx;
        ...
        minOp = addEligibleSelections(minOp, columNum);
    ...
    public Iterator<Record> execute() {
        ...
        int minIO = Integer.MAX_VALUE;

        for(Set<String> table: prevMap.keySet()){
            QueryOperator currOp = prevMap.get(table);
            int currentIO = currOp.estimateIOCost();
            if(minIO > currentIO){
                minIO = currentIO;
                this.finalOperator = currOp;
            }
        }

        // add selects, group by's and projects to our plan
        this.addGroupBy();
        this.addProject();
        this.addSort();
        this.addLimit();
