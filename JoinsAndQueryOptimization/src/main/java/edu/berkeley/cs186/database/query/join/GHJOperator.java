...
public class GHJOperator extends JoinOperator {
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        for (Record buildRecord: buildRecords) {
            JoinValue = buildRecord.getValue(buildColumnIndex);
        ...
        for (Record probeRecord: probeRecords) {
            JoinValue = probeRecord.getValue(probeColumnIndex);
            if (!hashTable.containsKey(JoinValue)) continue;
            // We have to join the right record with each left record with
            // a matching key
            for (Record lRecord : hashTable.get(JoinValue)) {
                Record joinedRecord;
                if(probeFirst){
                    joinedRecord = probeRecord.concat(lRecord);
                }
                else{
                    joinedRecord = lRecord.concat(probeRecord);
                }
