...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        if(!records.hasNext()) return null;//add
        // pass 0
        ...
        Run singleSortedRun = makeRun(temp);

        return singleSortedRun;
    }
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        ArrayList<BacktrackingIterator> iterList = new ArrayList<BacktrackingIterator>();
        Run singleSortedRun = makeRun();
        for (int i = 0; i< runs.size();i++){
            if(runs.get(i).iterator().hasNext()){
                pair = new Pair(runs.get(i).iterator().next(),i);
                iterList.add(runs.get(i).iterator());
                iterList.get(i).next();
                queue.add(pair);
            }
        }
        while(!queue.isEmpty()){
            assert queue.size()<= runs.size();
            Pair<Record,Integer> smallest = queue.poll();
            singleSortedRun.add(smallest.getFirst());
            if(iterList.get(smallest.getSecond()).hasNext()){
                queue.add(new Pair(iterList.get(smallest.getSecond()).next(),smallest.getSecond()));
            }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        List<Run> runAfterMerge = new ArrayList<>();
        int numOfRuns = (int)Math.ceil((double)runs.size()/(this.numBuffers-1));
        if(numOfRuns==0) return null;// add
        for(int i = 0; i<numOfRuns;i++){
            List<Run> group;
            if(i == numOfRuns-1){
                group = runs.subList(i*(this.numBuffers-1),runs.size());
            }
            else{
                group = runs.subList(i*(this.numBuffers-1),(i+1)*(this.numBuffers-1));
            }
            Run singleSortedRun =mergeSortedRuns(group);
            runAfterMerge.add(singleSortedRun);
        }
        return runAfterMerge;
    }
    ...
    public Run sort() {
        ...
        // TODO(proj3_part1): implement
        if(!sourceIterator.hasNext()) return makeRun();//add
...
//            System.out.println(sortedRuns.size());
    ...
    public Run makeRun(List<Record> records) {
        if(records.size()==0) return null;//add
